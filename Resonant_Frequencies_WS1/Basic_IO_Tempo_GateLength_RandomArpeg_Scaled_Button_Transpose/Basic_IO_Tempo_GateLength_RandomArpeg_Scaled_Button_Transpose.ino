// Simple example of playing a random  MIDI arpeggios in a certain scale on channel 1 in a tempo based on the level of a potentiometer ( sensor) attached to A0;
...
int playing; //This wil hold the last note played 
...
void loop()
{
    ...
    delay(gateLength);		    // Wait for the amount of gate length
    MIDI.sendNoteOff(noteB, 0, channel+1);  // Stop the note
    delay(tempo /3);

    MIDI.sendNoteOn(noteC, velocity, channel);    // Send the note with a velocity on a channel
    delay(gateLength);		    // Wait for the amount of gate length
    MIDI.sendNoteOff(noteC, 0, channel+2);  // Stop the note
    delay(tempo /3);
