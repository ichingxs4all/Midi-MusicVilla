...
void handleControlChange(byte channel, byte number, byte value) {
  MIDI.sendControlChange(number, value, channel);  //Send the control change value
}
...
void handleClock() {
  if (playStatus == 1) {
    ...
    if (ccValue != lastCcValue) {
      handleControlChange(channel, ccNo, ccValue); 
      lastCcValue = ccValue;  //Store this value for comparising later
    }
    ...
    if (click == step) {
      ...
      if (randomiserOn == 1) {                                    //Check if the touch button is touched to enable the randomiser
        ...
        handleNoteOn(channel, note[seqStep], velocity);  //Play the calculated note
        lastNotePlayed = note[seqStep];                  // Store the last played note
        seqStep++;                                       //Advance a step in the sequence
        if (seqStep > numberOfNotes) seqStep = 0;        //If the end of the sequence is reached start from the beginning
      }
